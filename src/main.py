...
def experiment():
    ...
    # The defaulted preprocessed data is the one we preprocessed ourselves. 
    # If you want to use the data preprocessed by the instructions from the notebook on GitHub, set dl.load_data(use_preprocessed_data_ipynb=True).
    sentences = dl.load_data()

    skipgram_model, skipgram_time = skipgram.train_skipgram_model(sentences)
    print(skipgram_time)

    embeddings_sppmi_svd, sppmi_svd_time = SPPMI.train_sppmi_svd_model(sentences, device)
    print(sppmi_svd_time)

    assert device.type == "cuda", "GloVe model will meet a segmentation fault when trained on CPU!"
    ...
    # To do cosine similarity evaluation, we decide to use two types of word pairs: both words from the same category or from different categories separately.
